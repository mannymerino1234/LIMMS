                    ...
                    <div class="container">
                        ...
                        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="get" class="form-inline mb-3">
                            <div class="row">
                                ...
                                <div class="col-sm-4">
                                    <div class="form-group">
                                        <input class="form-control" type="text" id="search_account_id" name="search_account_id" placeholder="Account ID" value="<?php echo isset($_GET['search_account_id']) ? htmlspecialchars($_GET['search_account_id']) : ''; ?>">
                                    </div>
                                </div>
                            </div>
                        ...
                        <table class="table table-bordered">
                            ...
                            <tbody>
                            <?php
                                // Fetch items from the database with status "Confirmed" and search functionality
                                ...
                                $search_account_id = isset($_GET['search_account_id']) ? '%' . $_GET['search_account_id'] . '%' : '%';

                                $sql = "SELECT t.transaction_id, i.name AS item_name, a.id_number AS id_number, t.quantity, t.return_date, t.status
                                        FROM transaction t
                                        INNER JOIN items i ON t.item_id = i.item_id
                                        INNER JOIN accounts a ON t.account_id = a.account_id
                                        WHERE t.status = 'Confirmed Request'
                                        AND t.transaction_id LIKE ?
                                        AND i.name LIKE ?
                                        AND a.id_number LIKE ?";
                                $stmt = mysqli_prepare($conn, $sql);
                                mysqli_stmt_bind_param($stmt, 'sss', $search_transaction_id, $search_item_name, $search_account_id);
                                ...
                                if (mysqli_num_rows($result) > 0) {
                                    while ($row = mysqli_fetch_assoc($result)) {
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <form id="confirmForm<?php echo $row['transaction_id']; ?>" action="update_status_return.php" method="post" onsubmit="return confirmRequest(<?php echo $row['transaction_id']; ?>)">
